...
namespace Shipit
{
    public partial class OrderBooking : Form
    {
        ...
        //    ordrbookin.Style = tbl_bookdata.Rows[rownumber].Cells[8].Value.ToString().Trim ();
            int styleindex = tbl_bookdata.Columns["Stylecombo"].Index;
            ordrbookin.Style = tbl_bookdata.Rows[rownumber].Cells[styleindex].Value.ToString().Trim();
         ...
         curdatacontext.OrderBooking_tbls.InsertOnSubmit(ordrbookin);
          ...
          Transaction.ClsEmailer.bookingnotifier(Program.uername, buyer, tbl_bookdata.Rows[rownumber].Cells[1].Value.ToString(), cmb_year.Text, cmb_month.Text, tbl_bookdata.Rows[rownumber].Cells[styleindex].Value.ToString(), tbl_bookdata.Rows[rownumber].Cells[6].Value.ToString());
...
FROM            Factory_Master f", con);
        ...
        public void loadStylecombo()
        {

            CourierDataDataContext courdatacontext = new CourierDataDataContext(Program.ConnStr);
            var q = from buyerstyle in courdatacontext.BuyerStyleMasters
                    select buyerstyle;

            Stylecombo.DataSource = q.ToList ();
            Stylecombo.DisplayMember = "BuyerStyle";
            Stylecombo.ValueMember = "BuyerStyle";
